...
module detector_header 
    ...
    type, private  :: detector_info_tp
        ...
        integer, public                              :: ichannel = 1            ! "1"-kappa*sigmaf, "2"-nu*sigmaf
...
contains
    ...
    ! 
    !===============================================================================================
    subroutine Get_Detector_Value (this, ns, geom, mesh, xsec, flux)
        ...
        do i = 1, this%ndet 
            ...
            if (idx < 0)  then
                do ig = 1, ns%state%ng
                    if (this%detpoint(i)%ichannel == 1)  val = val + flux%ngs(ig)%scalar(ir,ia) * xsec%matrixs(iz, ia)%sigma_f_kappa(ig)
                    if (this%detpoint(i)%ichannel == 2)  val = val + flux%ngs(ig)%scalar(ir,ia) * xsec%matrixs(iz, ia)%sigma_f_nu(ig)
                end do 
            else
                if (ir <= 0)  then
                    do ig = 1, ns%state%ng
