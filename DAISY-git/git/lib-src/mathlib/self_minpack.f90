...
!===================================================================================================
!
!   self version of minpack: Least Squares Minimization of Vector Functions 
!---------------------------------------------------------------------------------------------------
!   Public subroutine lists:    CHKDER                          checks the gradients of M functions of N variables. 
...
contains
    ...
    subroutine chkder (m, n, x, fvec, fjac, ldfjac, xp, fvecp, mode, err)
        ...
        ! MODE = 2
            ...
            do i = 1, m
                ...
                if ( fvec(i) /= 0.0D+00 .and. fvecp(i) /= 0.0D+00 .and. ABS ( fvecp(i)-fvec(i)) >= epsf * ABS ( fvec(i) ) ) then
    ...
    subroutine dogleg (n, r, lr, diag, qtb, delta, x)
        ...
        ! The Gauss-Newton direction is not acceptable. Calculate the scaled gradient direction.
        l = 1
        do j = 1, n
            temp = qtb(j)
            do i = j, n
        ...
        ! Calculate the norm of the scaled gradient. Test for the special case in which the scaled gradient is zero.
        gnorm = enorm ( n, wa1 )
        sgnorm = 0.0D+00
        alpha = delta / qnorm
        ...
        if ( gnorm /= 0.0D+00 ) then
            ...
            ! The scaled gradient direction is not acceptable. Calculate the point along the dogleg at which the quadratic is minimized.
            if ( sgnorm < delta ) then
        
            bnorm = enorm ( n, qtb )
            temp = ( bnorm / gnorm ) * ( bnorm / qnorm ) * ( sgnorm / delta )
            temp = temp - ( delta / qnorm ) * ( sgnorm / delta)**2 + SQRT ( ( temp - ( delta / qnorm ) )**2     &
                &   + ( 1.0D+00 - ( delta / qnorm )**2 ) * ( 1.0D+00 - ( sgnorm / delta )**2 ) )
...
30      continue
        ...
        !  On the first iteration, if MODE is 1, scale according to the norms of the columns of the initial jacobian.
        if ( iter == 1 ) then
            if ( mode /= 2 ) then
...
180     continue
        ...
        ! Calculate the rank one modification to the jacobianand update QTF if necessary.
        do j = 1, n
...
300     continue
    ...
    subroutine hybrd1 ( fcn, n, x, fvec, tol, info )
        ...
        call hybrd ( fcn, n, x, fvec, xtol, maxfev, ml, mu, epsfcn, diag, mode, factor, nprint, info, nfev, fjac, ldfjac, r, lr, qtf )
    ...
    subroutine hybrj (fcn, n, x, fvec, fjac, ldfjac, xtol, maxfev, diag, mode, factor, nprint, info, nfev, njev, r, lr, qtf)
        ...
        if ( ldfjac < n .or. xtol < 0.0D+00 .or. maxfev <= 0 .or. factor <= 0.0D+00 .or. lr < ( n * ( n + 1 ) ) / 2 ) then
            if ( iflag < 0 ) then
                info = iflag
            end if
            iflag = 0
...
300     continue
    ...
    subroutine hybrj1 (fcn, n, x, fvec, fjac, ldfjac, tol, info)
        ...
        call hybrj ( fcn, n, x, fvec, fjac, ldfjac, xtol, maxfev, diag, mode, factor, nprint, info, nfev, njev, r, lr, qtf )
    ...
    subroutine lmder (fcn, m, n, x, fvec, fjac, ldfjac, ftol, xtol, gtol, maxfev, diag, mode, factor, nprint, info, nfev, njev, ipvt, qtf)
        ...
        if ( ldfjac < m .or. ftol < 0.0D+00 .or. xtol < 0.0D+00 .or. gtol < 0.0D+00  .or. maxfev <= 0 .or. factor <= 0.0D+00 ) then
            go to 300
        end if
...
200     continue
        ...
        if ( ABS ( actred) <= ftol .and. prered <= ftol .and. 0.5D+00 * ratio <= 1.0D+00 ) then
            info = 1
        end if
        ...
        if ( ABS ( actred) <= ftol .and. prered <= ftol .and. 0.5D+00 * ratio <= 1.0D+00 .and. info == 2 ) then
            info = 3
        end if
        ...
        if ( ABS ( actred ) <= epsmch .and. prered <= epsmch .and. 0.5D+00 * ratio <= 1.0D+00 ) then
            info = 6
        end if
...
300     continue
    ...
    subroutine lmder1 (fcn, m, n, x, fvec, fjac, ldfjac, tol, info)
        ...
        call lmder ( fcn, m, n, x, fvec, fjac, ldfjac, ftol, xtol, gtol, maxfev, diag, mode, factor, nprint, info, nfev, njev, ipvt, qtf )
...
300     continue
    ...
    subroutine lmdif1 ( fcn, m, n, x, fvec, tol, info )
        ...
        call lmdif (fcn, m, n, x, fvec, ftol, xtol, gtol, maxfev, epsfcn, diag, mode, factor, nprint, info, nfev, fjac, ldfjac, ipvt, qtf)
...
30      continue
        ...
        ! Compute the QR factorization of the jacobian matrix calculated one row at a time, while simultaneously forming Q'* FVEC and storing
        ...
        ! If the jacobian is rank deficient, call QRFAC to reorder its columns and update the components of QTF.
        sing = .FALSE.
        do j = 1, n
            if ( fjac(j,j) == 0.0D+00 ) then
                sing = .TRUE.
        ...
        ! if mode is 1, scale according to the norms of the columns of the initial jacobian.
        ! calculate the norm of the scaled X, initialize the step bound delta.
        if ( iter == 1 ) then
            if ( mode /= 2 ) then
...
340     continue
    ...
    subroutine lmstr1 (fcn, m, n, x, fvec, fjac, ldfjac, tol, info)
        ...
        call lmstr ( fcn, m, n, x, fvec, fjac, ldfjac, ftol, xtol, gtol, maxfev, diag, mode, factor, nprint, info, nfev, njev, ipvt, qtf )
    ...
    subroutine qrsolv ( n, r, ldr, ipvt, diag, qtb, x, sdiag )
        ...
        do j = 1, n
            ...
            if ( diag(l) /= 0.0D+00 ) then
                ...
                ! The transformations to eliminate the row of D modify only a single element of Q'*B
    ...
    subroutine r1updt (m, n, s, ls, u, v, w, sing)
        ...
        do j = n-1, 1, -1
            ...
            if ( v(j) /= 0.0D+00 ) then
                ...
                ! Apply the transformation to V and store the information necessary to recover the Givens rotation.
                v(n) = sin * v(j) + cos * v(n)
                v(j) = tau
    ...
    subroutine timestamp ()
        ...
        write ( *, '(i2.2,1x,a,1x,i4,2x,i2,a1,i2.2,a1,i2.2,a1,i3.3,1x,a)' ) d, TRIM(month(m)), y, h, ':', n, ':', s, '.', mm, TRIM (ampm)
        
        return
    end subroutine timestamp

end module self_minpack
