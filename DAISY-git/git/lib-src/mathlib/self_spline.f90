...
!===================================================================================================
!
!   self version of spline: Interpolation and Approximation of Data  
!---------------------------------------------------------------------------------------------------
!   Public subroutine lists:    BASIS_FUNCTION_B_VAL                evaluates the B spline basis function.
...
contains
    ...
    subroutine basis_function_b_val (tdata, tval, yval)
        ...
        if ( tval < tdata(2) ) then
            ...
            yval = ( ( ( - 3.0D+00  * u + 3.0D+00 ) * u + 3.0D+00 ) * u + 1.0D+00 ) / 6.0D+00
        else if ( tval < tdata(4) ) then
            yval = ( ( ( + 3.0D+00  * u - 6.0D+00 ) * u + 0.0D+00 ) * u + 4.0D+00 ) / 6.0D+00
    ...
    subroutine basis_function_beta_val (beta1, beta2, tdata, tval, yval)
        ...
        if ( tval < tdata(2) ) then
            ...
            a = beta2 + 4.0D+00*beta1 + 4.0D+00*beta1*beta1 + 6.0D+00*(1.0D+00 - beta1*beta1 ) - 3.0D+00*(2.0D+00 + beta2 + 2.0D+00*beta1 ) + 2.0D+00*(1.0D+00 + beta2 + beta1 + beta1*beta1)
            b = - 6.0D+00*(1.0D+00 - beta1*beta1 ) + 6.0D+00*( 2.0D+00 + beta2 + 2.0D+00*beta1 ) - 6.0D+00*(1.0D+00 + beta2 + beta1 + beta1*beta1)
            c = - 3.0D+00*(2.0D+00 + beta2 + 2.0D+00*beta1 ) + 6.0D+00*(1.0D+00 + beta2 + beta1 + beta1*beta1)
        ...
        yval = yval / ( 2.0D+00 + beta2 + 4.0D+00 * beta1 + 4.0D+00 * beta1**2 + 2.0D+00 * beta1**3 )
    ...
    subroutine basis_matrix_b_uni ( mbasis )
        ...
        ! In the following statement, the matrix appears as though it has been transposed.
        mbasis(1:4,1:4) = real (RESHAPE((/ -1,  3, -3,  1,                &
                                        &   3, -6,  0,  4,                &
                                        &  -3,  3,  3,  1,                &
                                        &   1,  0,  0,  0 /),             &
                                        &   (/ 4, 4 /) ), kind = 8 ) / 6.0D+00
    ...
    subroutine basis_matrix_beta_uni (beta1, beta2, mbasis)
        ...
        mbasis(1,2) =   2.0D+00 * beta2 + 2.0 * beta1 * ( beta1 * beta1 + beta1 + 1.0D+00 )
        mbasis(1,3) = - 2.0D+00 * ( beta2 + beta1 * beta1 + beta1 + 1.0D+00 )
        mbasis(1,4) =   2.0D+00
        
        mbasis(2,1) =   6.0D+00 * beta1 * beta1 * beta1
        mbasis(2,2) = - 3.0D+00 * beta2 - 6.0D+00 * beta1 * beta1 * ( beta1 + 1.0D+00 )
        ...
        delta = ( ( 2.0D+00 * beta1 + 4.0D+00 ) * beta1 + 4.0D+00 ) * beta1 + 2.0D+00 + beta2
    ...
    subroutine basis_matrix_overhauser_nonuni (alpha, beta, mbasis)
        ...
        mbasis(2,3) = ( 2.0D+00 * ( 1.0D+00 - alpha ) - beta * ( 1.0D+00 - 2.0D+00 * alpha ) ) / ( 1.0D+00 - beta )
    ...
    subroutine parabola_val2 ( dim_num, ndata, tdata, ydata, left, tval, yval )
        ...
        ! Construct and evaluate a parabolic interpolant for the data in each dimension.
        do i = 1, dim_num
            y1 = ydata(i,left)
            y2 = ydata(i,left+1)
            y3 = ydata(i,left+2)
    ...
    subroutine r8vec_bracket3 ( n, t, tval, left )
        ...
        !  CASE 1: TVAL < T(LEFT): Search for TVAL in [T(I), T(I+1)] for intervals I = 1 to LEFT-1.
        if ( tval < t(left) ) then
            if ( left == 1 ) then
                return
            else if ( left == 2 ) then
        ...
        ! CASE2: T(LEFT+1) < TVAL: Search for TVAL in {T(I),T(I+1)] for intervals I = LEFT+1 to N-1.
        else if ( t(left+1) < tval ) then
            if ( left == n - 1 ) then
                return
        ...
        !  CASE3: T(LEFT) <= TVAL <= T(LEFT+1): T is in [T(LEFT), T(LEFT+1)], as the user said it might be.
        else
    ...
    subroutine r8vec_even ( n, alo, ahi, a )
        ...
        if ( n == 1 ) then
            ...
            do i = 1, n
                a(i) = ( real ( n - i, kind = 8 ) * alo + real ( i - 1, kind = 8 ) * ahi ) / real ( n - 1, kind = 8 )
            end do
        end if
    ...
    subroutine spline_b_val ( ndata, tdata, ydata, tval, yval )
        ...
        ! Evaluate the 5 nonzero B spline basis functions in the interval, weighted by their corresponding data values.
        u = ( tval - tdata(left) ) / ( tdata(right) - tdata(left) )
        yval = 0.0D+00

        ! B function associated with node LEFT - 1, (or "phantom node"), evaluated in its 4th interval.
        bval = ( ( ( - 1.0D+00 * u + 3.0D+00 ) * u - 3.0D+00 ) * u + 1.0D+00 ) / 6.0D+00
        if ( 0 < left-1 ) then
        ...
        bval = ( ( ( 3.0D+00 * u - 6.0D+00 ) * u + 0.0D+00 ) * u + 4.0D+00 ) / 6.0D+00
        yval = yval + ydata(left) * bval
        
        ! B function associated with node RIGHT, evaluated in its second interval.
        bval = ( ( ( - 3.0D+00 * u + 3.0D+00 ) * u + 3.0D+00 ) * u + 1.0D+00 ) / 6.0D+00
        yval = yval + ydata(right) * bval
        
        ! B function associated with node RIGHT+1, (or "phantom node"), evaluated in its first interval.
        bval = u**3 / 6.0D+00
    ...
    subroutine spline_beta_val ( beta1, beta2, ndata, tdata, ydata, tval, yval )
        ...
        ! Evaluate the 5 nonzero beta spline basis functions in the interval, weighted by their corresponding data values.
        u = ( tval - tdata(left) ) / ( tdata(right) - tdata(left) )
        
        delta = ( ( 2.0D+00 * beta1 + 4.0D+00 ) * beta1 + 4.0D+00 ) * beta1 + 2.0D+00 + beta2
        yval = 0.0D+00
        
        ! Beta function associated with node LEFT - 1, (or "phantom node"), evaluated in its 4th interval.
        ...
        ! Beta function associated with node LEFT, evaluated in its third interval.
        a = beta2 + ( 4.0D+00 + 4.0D+00 * beta1 ) * beta1
        b = - 6.0D+00 * beta1 * ( 1.0D+00 - beta1 ) * ( 1.0D+00 + beta1 )
        c = ( ( - 6.0D+00 * beta1 - 6.0D+00 ) * beta1 + 0.0D+00 ) * beta1 - 3.0D+00 * beta2
        d = ( ( + 2.0D+00 * beta1 + 2.0D+00 ) * beta1 + 2.0D+00 ) * beta1 + 2.0D+00 * beta2
        
        bval = ( a + u * ( b + u * ( c + u * d ) ) ) / delta
        yval = yval + ydata(left) * bval
        
        ! Beta function associated with node RIGHT, evaluated in its second interval.
        a = 2.0D+00
        b = 6.0D+00 * beta1
        ...
        ! Beta function associated with node RIGHT+1, (or "phantom node"), evaluated in its first interval.
    ...
    !===============================================================================================
    subroutine spline_bezier_val (dim_num, interval_num, data_val, point_num, point_t, point_val)
    ...
    subroutine spline_cubic_set ( n, t, y, ibcbeg, ybcbeg, ibcend, ybcend, ypp )
        ...
        ! Special case: N = 2, IBCBEG = IBCEND = 0.
    ...
    subroutine spline_cubic_val ( n, t, y, ypp, tval, yval, ypval, yppval )
        ...
        ! Determine the interval [T(LEFT), T(RIGHT)] that contains TVAL. Values below T(1) or above T(N) use extrapolation.
        ...
        yval = y(left) + dt * ((y(right) - y(left)) / h - (ypp(right) / 6.0D+00 + ypp(left) / 3.0D+00)*h + dt*(0.5D+00 * ypp(left) + dt*((ypp(right) - ypp(left)) / (6.0D+00*h))))
        ypval = (y(right) - y(left)) / h  - (ypp(right) / 6.0D+00 + ypp(left) / 3.0D+00)*h + dt*(ypp(left) + dt * ( 0.5D+00 * (ypp(right) - ypp(left)) / h))
    ...
    subroutine spline_cubic_val2 ( n, t, y, ypp, left, tval, yval, ypval, yppval )
        ...
        ! What you want from R8VEC_BRACKET3 is that TVAL is to be computed by the data in interval [T(LEFT), T(RIGHT)].
        call r8vec_bracket3 ( n, t, tval, left )
        right = left + 1
        
        ! In the interval LEFT, the polynomial is in terms of a normalized coordinate  ( DT / H ) between 0 and 1.
        dt = tval - t(left)
        h = t(right) - t(left)
    ...
    subroutine spline_linear_int ( ndata, tdata, ydata, a, b, int_val )
        ...
        ! Find the interval [ TDATA(A_LEFT), TDATA(A_RIGHT) ] that contains, or is nearest to, A.
        call r8vec_bracket ( ndata, tdata, a_copy, a_left, a_right )
        
        ! Find the interval [ TDATA(B_LEFT), TDATA(B_RIGHT) ] that contains, or is nearest to, B.
        ...
        if ( a_left == b_left ) then
            ...
            yp = ( ydata(a_right) - ydata(a_left) ) / ( tdata(a_right) - tdata(a_left) )
        ...
        !  TDATA(B_LEFT)   to B.
        !
        !  Use the fact that the integral of a linear function is the value of the function at the midpoint times the width of the interval.
        ...
        do i_left = a_right, b_left - 1
            ...
            yp = ( ydata(i_left+1) - ydata(i_left) ) / ( tdata(i_left+1) - tdata(i_left) )
        ...
        yp = ( ydata(b_right) - ydata(b_left) ) / ( tdata(b_right) - tdata(b_left) )
    ...
    subroutine spline_linear_intset ( n, int_x, int_v, data_x, data_y )
        ...
        a(3,1:n-2) = 1.0D+00 - ( 0.5D+00 * ( data_x(2:n-1) + int_x(2:n-1) ) - data_x(1:n-2) ) / ( data_x(2:n-1) - data_x(1:n-2) )
        a(3,n-1) = 0.0D+00
        a(3,n) = 0.0D+00
        
        a(2,1) = int_x(2) - int_x(1)
        a(2,2:n-1) = 1.0D+00 + (0.5D+00*(data_x(2:n-1) + int_x(2:n-1)) - data_x(1:n-2)) / (data_x(2:n-1) - data_x(1:n-2))   &
                &   - (0.5D+00*(data_x(2:n-1) + int_x(3:n)) - data_x(2:n-1)) / (data_x(3:n) - data_x(2:n-1))
        a(2,n) = int_x(n+1) - int_x(n)
        
        a(1,1) = 0.0D+00
        a(1,2) = 0.0D+00
        a(1,3:n) = ( 0.5D+00 * ( data_x(2:n-1) + int_x(3:n) ) - data_x(2:n-1) ) / ( data_x(3:n) - data_x(2:n-1) )
    ...
    subroutine spline_overhauser_nonuni_val ( ndata, tdata, ydata, tval, yval )
        ...
        if ( left == 1 ) then
            d21 = SQRT ( ( tdata(2) - tdata(1) )**2 + ( ydata(2) - ydata(1) )**2 )
            d32 = SQRT ( ( tdata(3) - tdata(2) )**2 + ( ydata(3) - ydata(2) )**2 )
            alpha = d21 / ( d32 + d21 )
            ...
            d21 = SQRT ( ( tdata(left) - tdata(left-1) )**2 + ( ydata(left) - ydata(left-1) )**2 )
            d32 = SQRT ( ( tdata(left+1) - tdata(left) )**2 + ( ydata(left+1) - ydata(left) )**2 )
            d43 = SQRT ( ( tdata(left+2) - tdata(left+1) )**2 + ( ydata(left+2) - ydata(left+1) )**2 )
            alpha = d21 / ( d32 + d21 )
            beta  = d32 / ( d43 + d32 )
            ...
            d32 = SQRT ( ( tdata(ndata-1) - tdata(ndata-2) )**2 + ( ydata(ndata-1) - ydata(ndata-2) )**2 )
            d43 = SQRT ( ( tdata(ndata) - tdata(ndata-1) )**2 + ( ydata(ndata) - ydata(ndata-1) )**2 )
            beta  = d32 / ( d43 + d32 )
    ...
    subroutine spline_overhauser_val ( dim_num, ndata, tdata, ydata, tval, yval )
        ...
        ! Locate the abscissa interval T(LEFT), T(LEFT+1) nearest to or containing TVAL.
        call r8vec_bracket ( ndata, tdata, tval, left, right )
        ...
        if ( left == 1 ) then
            ...
            yval(1:dim_num) =  ((tdata(right) - tval) * yl(1:dim_num) + (tval - tdata(left) ) * yr(1:dim_num) ) / ( tdata(right) - tdata(left) )
    ...
    subroutine spline_pchip_val ( n, x, f, d, ne, xe, fe )
        ...
        ! Loop over intervals. The interval index is IL = IR-1. The interval is X(IL) <= X < X(IR).
        j_first = 1
        ir = 2
        do
            ...
            if ( nj /= 0 ) then
                ...
                call chfev ( x(ir-1), x(ir), f(ir-1), f(ir), d(ir-1), d(ir), nj, xe(j_first:j-1), fe(j_first:j-1), next, ierc )
